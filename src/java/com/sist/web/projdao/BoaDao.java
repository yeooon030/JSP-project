...
import com.sist.web.proj.Boa;
import com.sist.web.proj.Mem;

public class BoaDao {

	private static Logger logger = LogManager.getLogger(BoaDao.class);
	
	//게시물 리스트 조회
	public List<Boa> boardList(Boa search)
	{
		List<Boa> list = new ArrayList<Boa>();
		...
		sql.append("                      NVL(A.BOA_LIKE, 0) BOA_LIKE ");
	...
	//총 게시물 조회
	public long totalBoaCount(Boa search)
	{
		long totalCount = 0;
	...
	public int uploadBoa(Boa boa)
	{
		...
		sql.append("(BOA_NUM, MEM_ID, MEM_PWD, MEM_NAME, BOA_TITLE, BOA_CONTENT, BOA_CNT, REG_DATE, BOA_LIKE ) ");
	...
	//상세게시물 조회
	public Boa viewBoa(long boaNum)
	{
		Boa boa = null;
		...
		sql.append("       NVL(TO_CHAR(A.REG_DATE,'YYYY.MM.DD HH24:MI:SS'), '') REG_DATE, ");
		sql.append("       NVL(A.BOA_LIKE, '') BOA_LIKE");
		sql.append("  FROM BOA_TBL A, MEM_TBL B ");
		sql.append(" WHERE A.MEM_ID = B.MEM_ID ");
	...
	//게시물 추천
	public int likeBoa(long boaNum)
	{
		Connection conn = null;
		PreparedStatement pstmt = null;
		StringBuilder sql = new StringBuilder();
		...
		sql.append("UPDATE BOA_TBL ");
		sql.append("   SET BOA_LIKE = BOA_LIKE + 1 ");
		sql.append(" WHERE BOA_NUM = ? ");
		...
		{
			logger.error("[BoaDao] likeBoa SQLException", e);
		}
		finally
		{
			DBManager.close(pstmt, conn);
	...
	//회원관리 리스트 조회
		public List<Mem> MemList(Mem search)
		{
		...
		//총 회원수 조회
		public long totalMemCount(Mem search)
		{
			long totalCount = 0;
